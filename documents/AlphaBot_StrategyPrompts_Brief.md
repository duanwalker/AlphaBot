# AlphaBot — Strategy-Aware System Prompts Brief
*Prepared by Claude (Anthropic) as lead architect*
*Date: May 2026*
*Scope: Complete rewrite of systemPrompts.js,
        enriched sentiment context, strategy-aware
        assistant for all options strategies*

---

## Overview

This brief upgrades AlphaBot's AI assistant to reason
correctly about all supported trading strategies. The
current system prompt gives Claude the user's strategy
list but no guidance on HOW to reason about each
strategy, WHEN sentiment is reliable vs noise, or HOW
to cross-reference the user's existing positions.

The result is an assistant that gives generic advice
regardless of strategy. After this upgrade Claude will:

- Apply strategy-specific reasoning frameworks
- Weight sentiment correctly by market cap category
- Cross-reference existing positions before suggesting
- Distinguish between income, growth, and hedging goals
- Surface the right strategy for the current symbol
- Know which strategies require more experience
- Flag when a suggestion is outside the user's profile

---

## Codebase Context

**File to rewrite:** services/systemPrompts.js
  Current exports: getSingleSymbolPrompt, getMarketPrompt
  Keep same export names — no changes needed in server.js

**File to update:** services/compressionService.js
  Add compressSentimentContext() function that enriches
  the sentiment object passed to Claude with reliability
  metadata derived from market cap

**File to update:** server.js
  Update the assistant handler to pass enriched sentiment
  context and existing positions to the system prompt
  builder functions

**Current data available in /api/assistant handler:**
  - fundamentals (marketCap, peRatio, eps, beta, sector)
  - sentiment (score, signalStrength, trend, history)
  - positions (user's current holdings from Alpaca)
  - userProfile (activeStrategies, riskLevel, experience)
  - news headlines
...
Add a strategy registry to systemPrompts.js. This is
the single source of truth for all supported strategies.
New strategies can be added by adding one entry here
...
// ── Strategy Registry ──────────────────────────────────────
// Each strategy has:
//   id:          matches activeStrategies[] values
//   name:        human-readable name
//   category:    'income' | 'growth' | 'hedge' | 'passive'
//   complexity:  'beginner' | 'intermediate' | 'advanced'
//   minExperience: minimum options experience level required
//   capitalMin:  approximate minimum capital needed
//   timeCommitment: 'passive' | 'weekly' | 'daily'
...
The wheel is a systematic income strategy: sell CSPs
on stocks you want to own, collect premium. If assigned,
sell covered calls to generate more income until shares
are called away, then repeat.
...
Stock selection criteria:
  ...
  - Avoid: pre-earnings unless intentional, highly volatile
    biotech/clinical-stage companies, penny stocks

Strike selection for CSPs:
  - Target 0.20-0.30 delta (approximately 20-30% chance
    of being assigned at expiry)
  - Strike should be at or below a support level
  - Premium should be at least 1% of strike price for
    30-day expiry (12%+ annualized)
...
Position sizing:
  - Never allocate more than 20% of portfolio to
    one wheel position
  - Cash secured means having 100% of strike × contracts
    in buying power
...
When assigned (stock dropped below strike):
  - Do NOT panic sell — assignment means you bought at
    your predetermined price
  - Immediately evaluate selling covered calls
  - Strike for CC: above your cost basis
...
Sentiment role in wheel strategy:
  ...
  - HIGH value: sentiment rising + high signal strength
    → supports selling CSP (social conviction present)
  - HIGH value: sentiment falling + bearish trend
    → delay entry, wait for stabilization
  - LOW value for large caps (AAPL, MSFT, NVDA, AMZN)
    → sentiment lags price, use technicals instead
  - MEDIUM value for mid caps ($5B-$50B market cap)
  - HIGH value for small caps (< $5B market cap)

When to roll vs close:
  - Roll when: position is at 21 DTE with profit
    available, stock has moved against you but
    thesis still intact
  - Close when: 50% of max profit reached early
    (take profit, redeploy capital)
...
User owns shares and sells call options above current
price to generate monthly income. The premium is
collected immediately. If stock stays below strike at
expiry, keep premium and shares. If stock rises above
strike, shares are "called away" at the strike price.

CRITICAL: Always check if user already owns the symbol
before suggesting covered calls. If they own 100+ shares,
covered calls are directly applicable.
...
Strike selection:
  ...
  - Use sentiment to adjust: rising sentiment → further OTM
    (don't cap upside when momentum is building)

What to avoid:
  - DO NOT sell calls through earnings dates
    (IV crush works against you if stock moves up)
  - DO NOT sell calls if user expects near-term price surge
  - DO NOT sell calls below cost basis (creates loss risk)
  - Watch ex-dividend dates: if stock is called away before
    ex-div, user misses the dividend

Rolling covered calls:
  - If stock approaches strike before expiry:
    Roll up and out (buy back current call, sell
    further OTM at later expiry for net credit)
  - Roll to maintain position without being called away

Sentiment role in covered calls:
  - Moderate relevance
  - Bullish/rising sentiment → sell further OTM strike
    (protect upside participation)
  - Bearish/falling sentiment → sell closer to money
    (capture more premium when stock likely to stall)
...
Sell put options secured by enough cash to buy 100 shares
if assigned. Generates income while waiting for a stock
to reach your desired entry price.
...
Sentiment role:
  - Higher weight than covered calls since direction
    matters more for CSPs
  - Positive/stable sentiment → supports selling put
  - Falling/negative sentiment → wait for stabilization
  - Strongly bearish sentiment → avoid (stock may fall
    further, forcing assignment at bad price)
...
Sell an OTM put spread AND an OTM call spread
simultaneously. Profit when stock stays within a range
(between the two short strikes) until expiry.
...
Best conditions for iron condors:
  ...
  - Liquid options market (liquid ETFs are ideal:
    SPY, QQQ, IWM work very well)
  - Neutral sentiment — neither strongly bullish
    nor strongly bearish

Strike selection:
  - Short strikes at 0.15-0.20 delta each side
    (roughly 1-2 standard deviations OTM)
...
Adjustment and management:
  ...
  - Adjust if short strike threatened:
    Roll the threatened side out and away
  - Close entire position if loss reaches 2x premium
  - Never hold through earnings

Sentiment role:
  - CRITICAL for condor: need neutral sentiment
  - Strongly bullish sentiment → condor is wrong
    strategy (stock likely to break out upward)
  - Strongly bearish sentiment → condor is wrong
    strategy (stock likely to break down)
  - Mixed/neutral sentiment → supports condor
  - Best symbols: index ETFs where sentiment is
    less meaningful (market-wide neutral)

Experience requirement:
  - Only suggest to users with 'advanced' options
...
Defined risk bullish play — cheaper than buying a naked
call, with capped upside.
...
Best conditions:
  ...
  - Elevated IV makes naked calls expensive →
    spread reduces cost
...
Strike selection:
  ...
  - Target: spread costs 30-40% of width
    (max profit = 1.5x-2x the risk)

Sentiment role:
  - HIGH relevance — direction matters completely
  - Strong bullish sentiment + high signal → supports
...
Sentiment role:
  ...
  - Falling/bearish sentiment + high signal → supports
    bear put spread entry
  - For hedging: less sentiment-dependent —
    hedge when you're worried regardless of sentiment
`,
  },
...
Buy a put option on a stock you already own. Acts as
insurance — limits downside while keeping full upside.
...
Cost consideration:
  ...
  - Consider collar (sell covered call to fund put)
    to reduce net cost of protection

Sentiment role:
  - Moderately relevant
  - Falling/bearish sentiment on owned position →
    consider protective put
...
The covered call premium funds the put purchase.
Creates a defined range for the position.
...
Sentiment role:
  - Moderate — use collar when sentiment turns negative
    on a position you want to protect but not sell
`,
  },
...
Long-dated call options (1-2 years expiry) used as
leveraged stock replacement. Less capital than buying
shares outright with similar upside exposure.
...
Buy both a call and a put. Profit when stock makes
a large move in either direction. Direction agnostic.

Straddle: buy ATM call + ATM put (same strike)
Strangle: buy OTM call + OTM put (different strikes,
          cheaper but needs bigger move to profit)
...
CRITICAL: Buy BEFORE IV rises (before catalyst announced
  widely). Buying after IV spikes = overpaying.

Sentiment role:
  - LOW relevance for direction (strategy is neutral)
  - Use to gauge retail excitement around catalyst:
    High sentiment + catalyst approaching = more retail
    participation = potentially bigger move
  - Extreme sentiment either direction can signal
    move magnitude (crowd positioning)

Experience requirement:
  - Only for advanced users — significant risk of
    losing full premium if stock stays flat
`,
  },
...
Regular, scheduled purchases of broad market index ETFs
regardless of price. Removes emotion from investing.

SENTIMENT SIGNAL: NOT APPLICABLE for index ETFs.
VTI, SCHD, QQQ, SPY etc have no meaningful social
media sentiment signal. Do NOT cite sentiment scores
for these symbols — they reflect price, not lead it.
...
DCA guidance:
  ...
  - Increase purchases during significant drawdowns
    (VIX > 30 = consider accelerating DCA)
...
Build a portfolio of dividend-paying stocks and ETFs
to generate regular passive income.

SENTIMENT SIGNAL: LOW relevance for most dividend stocks.
Dividend stocks (utilities, REITs, consumer staples) are
heavily institutional-owned. Social sentiment lags price.
Focus on fundamentals and dividend health metrics.

Dividend health checklist (surface these):
  - Payout ratio: under 60% is sustainable
    (exception: REITs can pay up to 90%)
  - Dividend growth rate (CAGR over 5 years)
    Growing dividend = inflation protection
  - Consecutive years of dividend increases
    (Dividend Aristocrats = 25+ years)
  - Free cash flow coverage of dividend
    (FCF > dividend payment = healthy)
...
Interest rate context:
  - Rising rates → dividend stocks fall in value
...
Buy and hold companies with high revenue growth rates,
large addressable markets, and strong competitive moats.
Accept higher volatility for higher long-term returns.

SENTIMENT SIGNAL: MODERATE to HIGH relevance.
Growth stocks are often retail-driven (AI stocks, EV,
biotech). Social sentiment can lead price for smaller
growth names. Use carefully for large-cap growth.
...
Key metrics for growth stocks:
  ...
  - Price/Sales ratio vs peers (growth stocks often
    unprofitable — P/E less useful)
...
Valuation framework:
  ...
  - DCF analysis requires many assumptions — handle
...
Sentiment role for growth stocks:
  ...
  - New product/service launch + rising sentiment =
    potential catalyst play
  - Insider buying + positive sentiment = strong signal
  - Falling sentiment + multiple quarters of miss =
    time to reassess thesis
`,
  },
...
Defined-risk options strategies using combinations of
bought and sold options. Includes credit spreads
(income) and debit spreads (directional bets).

Credit spreads (income-focused):
  - Bull put spread: sell higher put, buy lower put
    → bullish/neutral, collect premium
  - Bear call spread: sell lower call, buy higher call
    → bearish/neutral, collect premium
  - Iron condor: bull put spread + bear call spread
    → neutral, collect premium on both sides

Debit spreads (directional):
  - Bull call spread: buy lower call, sell higher call
    → bullish, defined cost
  - Bear put spread: buy higher put, sell lower put
    → bearish, defined cost
...
Experience requirement:
  IMPORTANT: Only recommend spreads to users with
  'advanced' options experience in their profile.
  If experience is 'none', 'bought', or 'sold_calls'
  → explain the strategy exists but recommend
    starting with simpler strategies first.
...
// ── Sentiment Reliability Framework ──────────────────────────

function getSentimentReliability(marketCap, postVolume) {
  ...
  // Low post volume = unreliable regardless of market cap
  if (posts > 0 && posts < 20) {
    reliability = 'low';
...
## Part 2: Updated getSingleSymbolPrompt

Replace the existing function with this comprehensive version:
...
## Part 3: Updated getMarketPrompt
...
export function getMarketPrompt(userProfile = null) {
  ...
  // Build brief strategy summaries for market context
  const stratSummaries = activeStrategies
    .map(id => {
...
No specific symbol is in focus. Provide a market-wide
assessment relevant to the user's active strategies:
${stratSummaries || 'no strategies configured'}

MARKET MODE TASKS:
1) Assess overall market tone (risk-on vs risk-off)
2) Identify sector strength/weakness
3) VIX/volatility context:
   - High VIX (>25): income strategies yield more,
     size positions conservatively, good for premium
     sellers (wheel, iron condor, credit spreads)
   - Low VIX (<15): compressed premiums, consider
     debit strategies or waiting for better entry
4) Relate conditions to user's watchlist sentiment
5) Strategy-specific market observations:
   - For income strategies: is IV environment
     favorable for selling premium?
   - For DCA strategies: is this a buying opportunity?
   - For growth strategies: risk-on or risk-off?
6) Suggest positioning adjustments if warranted

SENTIMENT RELIABILITY NOTE:
For broad market discussion, individual stock sentiment
scores are less relevant. Focus on market-wide signals:
  - VIX trend (fear/greed)
  - Sector rotation patterns
  - Overall watchlist sentiment trend

OUTPUT FORMAT:
A) Market tone — 1-2 sentences
B) Volatility environment — implications for strategies
C) Sector breakdown — strong/weak
D) Strategy context — what this means for user's approach
E) Watchlist highlights — any symbols worth noting`;
}
```

---

## Part 4: Enrich Context Passed to getSingleSymbolPrompt

Update the `/api/assistant` handler in `server.js` to
extract and pass enriched context to the prompt builder.

```javascript
// In /api/assistant handler
// After fetching fundamentals, sentiment, positions:

// Build enriched context object
const enrichedContext = {};

// Market cap for sentiment reliability
if (fundamentals?.marketCap) {
  enrichedContext.marketCap = fundamentals.marketCap;
}

// Post volume from sentiment snapshot
if (latestSnapshot?.postCount ||
    latestSnapshot?.totalPosts) {
  enrichedContext.postVolume =
    latestSnapshot.postCount ||
    latestSnapshot.totalPosts;
}

// Existing position in this symbol
if (positions && resolved.symbol) {
  const pos = positions.find(p =>
    p.symbol === resolved.symbol
  );
  if (pos) {
    enrichedContext.existingPosition = {
      qty: pos.qty,
      avgCost: pos.avgEntryPrice,
      unrealizedPLPercent: pos.unrealizedPlpc
        ? (parseFloat(pos.unrealizedPlpc) * 100)
            .toFixed(1) + '%'
        : 'unknown',
    };
  }
...
// VIX from market snapshot if available
const spySnapshot = marketSnapshot?.find(
  s => s.symbol === 'SPY'
);
if (spySnapshot) {
  // Approximate VIX from SPY movement is not reliable
  // Use actual VIX if available in market data
  // For now pass null — add real VIX data source later
  enrichedContext.vixLevel = null;
}

// Update system prompt call to pass enriched context:
const systemPrompt = resolved.mode === 'single'
  ? getSingleSymbolPrompt(
      resolved.symbol,
      userProfile,
      enrichedContext    // ← ADD THIS PARAMETER
    )
...
## Part 5: Add compressSentimentForClaude

Add to `services/compressionService.js`:

```javascript
// New function — compresses sentiment for Claude context
// Includes reliability metadata derived from market cap
export function compressSentimentForClaude(
  snapshot,
  marketCap = null
) {
  if (!snapshot) return null;

  const score = snapshot.sentimentScore ||
                snapshot.score || null;
  const postVolume = snapshot.postCount ||
                     snapshot.totalPosts || null;
...
## Part 6: Strategy Selector UI Update

The Settings → Profile strategy selector should show ALL
strategies from the registry, not just the original seven.

Update `client/src/pages/Settings/Profile.jsx` or wherever
strategy selection is rendered:

```javascript
// Full strategy list for UI display
// Import or hardcode this list in the frontend
// (mirrors STRATEGY_REGISTRY keys)

const ALL_STRATEGIES = [
  // Income
  { id: 'wheel_strategy',     name: 'Wheel Strategy',
    category: 'Income', complexity: 'Intermediate' },
  { id: 'covered_calls',      name: 'Covered Calls',
    category: 'Income', complexity: 'Beginner' },
  { id: 'cash_secured_puts',  name: 'Cash-Secured Puts',
    category: 'Income', complexity: 'Intermediate' },
  { id: 'iron_condor',        name: 'Iron Condor',
    category: 'Income', complexity: 'Advanced' },
  { id: 'bull_call_spread',   name: 'Bull Call Spread',
    category: 'Income', complexity: 'Intermediate' },
  { id: 'bear_put_spread',    name: 'Bear Put Spread',
    category: 'Hedge',  complexity: 'Intermediate' },
  { id: 'options_spreads',    name: 'Options Spreads',
    category: 'Income', complexity: 'Advanced' },

  // Hedge
  { id: 'protective_put',     name: 'Protective Put',
    category: 'Hedge',  complexity: 'Beginner' },
  { id: 'collar',             name: 'Collar',
    category: 'Hedge',  complexity: 'Intermediate' },

  // Growth
  { id: 'leaps',              name: 'LEAPS',
    category: 'Growth', complexity: 'Intermediate' },
  { id: 'straddle_strangle',  name: 'Straddle / Strangle',
    category: 'Growth', complexity: 'Advanced' },
  { id: 'growth_investing',   name: 'Growth Investing',
    category: 'Growth', complexity: 'Intermediate' },

  // Passive
  { id: 'index_dca',          name: 'Index Fund DCA',
    category: 'Passive', complexity: 'Beginner' },
  { id: 'dividend_income',    name: 'Dividend Income',
    category: 'Passive', complexity: 'Beginner' },
];

// Group by category for display:
// Income | Hedge | Growth | Passive

// Show complexity badge on each:
// Beginner (green) | Intermediate (amber) | Advanced (red)

// Lock advanced strategies with a note:
// "Requires advanced options experience"
// Still selectable but Claude will flag experience gap
```

---

## Part 7: Update Onboarding Analyze Prompt

Update the `/api/profile/analyze` endpoint in `server.js`
to recommend from the full strategy list:

```javascript
// Update the system prompt in /api/profile/analyze:
system: `You are a financial strategy advisor for AlphaBot.
Analyze the user's questionnaire and recommend strategies.
...
```
Step 1: services/systemPrompts.js — full rewrite
        Add STRATEGY_REGISTRY with all 14 strategies
        Update getSingleSymbolPrompt signature +
          enrichedContext parameter
        Update getMarketPrompt
        Add getSentimentReliability helper
        Test: import and call both functions without error

Step 2: services/compressionService.js
        Add compressSentimentForClaude function
        Test: call with a sample snapshot + marketCap

Step 3: server.js — enrich context object
        Build enrichedContext from available data
        Pass to getSingleSymbolPrompt
        Update /api/profile/analyze system prompt
        Test: POST /api/assistant with AAPL
              Server logs show enrichedContext built
              Response references sentiment reliability

Step 4: client strategy selector UI
        Update Profile.jsx strategy selector to show
          all 14 strategies grouped by category
        Add complexity badges
        Test: Settings → Profile shows full strategy list

Step 5: Production build passes
Step 6: Manual test — ask AlphaBot:
        "Should I sell a covered call on AAPL?"
        Response should:
        - Reference existing AAPL position if user owns it
        - Note AAPL is large cap (low sentiment reliability)
        - Give specific strike/expiry suggestion
        - Follow covered_calls strategy guide

        "What index funds should I buy?"
        Response should:
        - NOT cite sentiment scores for ETFs
        - Reference SCHD, VTI, VIG appropriately
        - Follow index_dca strategy guide
```
...
Adding new strategies in the future requires only:
1. Add one entry to STRATEGY_REGISTRY in systemPrompts.js
2. Add one entry to ALL_STRATEGIES in the frontend
3. Add the strategy ID to /api/profile/analyze prompt

No other code changes needed. The registry pattern
ensures strategies are self-contained and consistent
across all surfaces (assistant, onboarding, profile,
insight cards).

---

*Architect: Claude (Anthropic) | Project: AlphaBot*
*Scope: Strategy-Aware System Prompts — All Strategies*
